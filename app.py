...
cols = ['№', 'Дата пункции', 'ID пациентки', 'год рождения',
       'Возраст', '№ попытки', 'КОММ/ОМС', 'Фаза стимуляции',# 'Препарат ФСГ',
       # 'Препарат ЛГ', 'Триггер овуляции', 'суммарная доза ФСГ, МЕ',
       'Пункция донора ооцитов', 'Репродуктолог ТВП', 'Эмбриолог ТВП',
       'Количество фолликулов', 'Получено всего ооцитов',
       'размораживание своих ооцитов', 'Размораживание ДО',
       'Количество размороженных ооцитов', 'Оплодотворение',
       'Эмбриолог оплодотворения', 'TESE', 'Разморозка эякулята/биоптата',
       'ГКИ', 'Суррогатная мать', #'Сперма донорская', 'Ооциты донорские',
       'Объем', #'Концентрация',
        'Общ. кол-во','Кол-во крио эмб-в', # 'ХГЧ','Количество плодов по УЗИ'
        ]
...
def get_table(df):
    return dash_table.DataTable(
            ...
            tooltip_conditional = [
            {'if': { 'filter_query': '{} > {} || {} < {}'.format("{"+col+"}", key[0]+key[1], "{"+col+"}", key[0]-key[1]), 'column_id': col },
                        'type': 'markdown','value': 'Отклоняется от нормы'} for col, key in col_means_dict.items()] +
            [{'if': {'filter_query': '{{{}}} is nil'.format(col), 'column_id': col},
                        'type': 'markdown', 'value': 'Пустая ячейка'} for col in df.columns]
            )
...
app.layout = html.Div([
        ...
        #     columns=[ {'name': i, 'id': i, 'deletable': True, 'selectable': True, 'format': Format(nully='N/A')} for i in df.columns],
        #     data=df.to_dict('records'), style_cell={'whiteSpace': 'normal', 'height': 'auto'}, editable=True,
        #     filter_action='native', sort_action='native', sort_mode='multi', #column_selectable='multi',
        #     row_selectable=False, row_deletable=True, selected_columns=[], selected_rows=[], page_action='native',
        #     page_current=0, page_size=30,
        #     style_data_conditional = [
        #     {'if': { 'filter_query': '{} > {} || {} < {}'.format("{"+col+"}", key[0]+key[1], "{"+col+"}", key[0]-key[1]), 'column_id': col },
        #         'color': 'white','fontWeight': 'bold', 'backgroundColor': 'tomato'} for col, key in col_means_dict.items()]+
        #     # [{'if': { 'filter_query': '{} < {}'.format("{"+col+"}", key[0]-key[1]), 'column_id': col },
        #     #     'color': 'white','fontWeight': 'bold', 'backgroundColor': 'tomato'} for col, key in col_means_dict.items()]+
        #     [{'if': {'filter_query': '{{{}}} is nil'.format(col), 'column_id': col},
        #         'color': 'white', 'fontWeight': 'bold', 'backgroundColor': 'yellow'} for col in df.columns]+
        #
...
def parse_contents(contents, filename, date):
    ...
    return html.Div([
        ...
        # dash_table.DataTable(
        #     id='datatable-interactivity',
        #     columns=[{'name': i, 'id': i, 'deletable': True, 'selectable': True, 'format': Format(nully='N/A')} for i in
        #              df.columns],
        #     data=df.to_dict('records'), style_cell={'whiteSpace': 'normal', 'height': 'auto'}, editable=True,
        #     filter_action='native', sort_action='native', sort_mode='multi', #column_selectable='multi',
        #     row_selectable='multi', row_deletable=True, selected_columns=[ ], selected_rows=[], page_action='native',
        #     page_current=0, page_size=30,
        #     style_data_conditional=[
        #     {'if': {'filter_query': '{} > {}'.format("{" + col + "}", key[0] + key[1]),
        #                                        'column_id': col},
        #                                 'color': 'white', 'fontWeight': 'bold', 'backgroundColor': 'tomato'} for
        #                                col, key in col_means_dict.items()] +
        #     [{'if': {'filter_query': '{} < {}'.format("{" + col + "}", key[0] - key[1]),
        #                                     'column_id': col},
        #                              'color': 'white', 'fontWeight': 'bold', 'backgroundColor': 'tomato'} for col, key
        #                             in col_means_dict.items()] +
        #     [{'if': {'filter_query': '{{{}}} is nil'.format(col), 'column_id': col},
        #                              'color': 'white', 'fontWeight': 'bold', 'backgroundColor': 'yellow'} for col in
        #                             df.columns] +
        #     [{'if': {'filter_query': '{Прогноз_Беременности} > 0',
        #                                     'column_id': 'Прогноз_Беременности'},
        #                              'color': 'white', 'fontWeight': 'bold', 'backgroundColor': 'green'}] +
        #    [{'if': {'filter_query': '{Прогноз_Беременности} < 1', 'column_id': 'Прогноз_Беременности'},
        #                              'color': 'white', 'fontWeight': 'bold', 'backgroundColor': 'tomato'}] +
        #    [{'if': {'filter_query': '{Вероятность_Беременности} < 0.5', 'column_id': 'Вероятность_Беременности'},
...
def update_charts(selected_column_list, region, anomal, level, anomal_range_list,
    ...
    #         id='datatable-interactivity',
    #         columns=[ {'name': i, 'id': i, 'deletable': True, 'selectable': True, 'format': Format(nully='N/A')} for i in df.columns],
    #         data=filtered_data.to_dict('records'), style_cell={'whiteSpace': 'normal', 'height': 'auto'}, editable=True,
    #         filter_action='native', sort_action='native', sort_mode='multi', #column_selectable='multi',
    #         row_selectable=False, row_deletable=True, selected_columns=selected_column_list, selected_rows=[], page_action='native',
    #         page_current=0, page_size=30,
    #        # tooltip_header={i: i for i in filtered_data.columns},
    #         style_data_conditional = [
    #         {'if': {'filter_query': '{} > {} || {} < {}'.format("{" + col + "}",key[0] + level*key[1],"{" + col + "}", key[0] - level*key[1]),'column_id': col},
    #          'color': 'white', 'fontWeight': 'bold', 'backgroundColor': 'tomato'} for col, key in col_means_dict.items()] +
    #         [{'if': {'filter_query': '{{{}}} is nil'.format(col), 'column_id': col},
    #             'color': 'white', 'fontWeight': 'bold', 'backgroundColor': 'yellow'} for col in df.columns]+
    #
    ...
    #                     'type': 'markdown', 'value': 'Отклоняется от нормы'} for col, key in col_means_dict.items()] +
    #         [{'if': {'filter_query': '{{{}}} is nil'.format(col), 'column_id': col},
    #                     'type': 'markdown', 'value': 'Пропуск в ячейке'} for col in df.columns],
